...
int main()
{
	...
	else
	{
		...
		for (i = 0; i < height; i++)
		{
			for (j = 0; j < width; j++)
			{
				...
				fread(&red1, 1, 1, image_file);
				fread(&green1, 1, 1, image_file);
				fread(&blue1, 1, 1, image_file);
				int x,y,z;
				x=red1;y=green1;z=blue1;
			                    // the Image is a
				// 24-bit BMP Image
				   int red,green,blue,gray;
                                                                                // red = temp>>16;
                                                                                // green = (temp & 0x00ff00)>>8;
                                                                                // blue = (temp & 0x0000ff);
				   img[i][j][0]=0;
				   img[i][j][1]=0;
				   img[i][j][2]=0;
                                                                                //gray  =red* 0.3 + green* 0.58 + blue * 0.11;
                                                                                gray  =x* 0.3 + y* 0.58 + z * 0.11;
 ...
 char result[50];
        ...
        for (l = 0; l < nodes; l++){
            sprintf(result,"%d",pix_freq[l].pix);
            strcat(result," ");
            strcat(result,pix_freq[l].code);
            strcat(result," ");
            fprintf(table,"%s",result);
        }
        fclose(table);
